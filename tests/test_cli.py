...
class CliTests(unittest.TestCase):
    ...
    def test_page_action_plan_cli_writes_json_and_markdown_reports(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            output_dir = Path(temp_dir) / "generated"
            output_dir.mkdir()
            dry_run_path = output_dir / "browser_dry_run.json"
            ...
            self.assertEqual(plan_result.returncode, 0, plan_result.stderr)
            self.assertIn("Page Action Plan", plan_result.stdout)
            self.assertTrue((output_dir / "page_action_plan.json").exists())
            self.assertTrue((output_dir / "page_action_plan.md").exists())
